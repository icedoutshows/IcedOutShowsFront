...
export class EventoServiceService {
  ...
  update(evento: Evento): Observable<Evento>{
    return this.http.put<Evento>(`${this.url}/${evento.id}`,evento,{headers:this.httpHeaders});
  }
  ...
  getEntradaPorId(idEvento: number, idEntrada: number): Observable<Entrada> {
    const url = `${this.url}/entrada?idEvento=${idEvento}&idEntrada=${idEntrada}`;
    return this.http.get<Entrada>(url);
  }

  getEntrada( idEntrada: number): Observable<Entrada> {
    const url = `${this.url}/entradaCompleta/${idEntrada}`;
    return this.http.get<Entrada>(url);
  }
